...
resource "aws_instance" "this" {
  ...
  tags = "${merge(map("Name", (length(var.name_array) != 0) ? var.name_array[count.index] : (var.instance_count > 1) || (var.use_num_suffix == "true") ? format("%s-%d", var.name, count.index+1) : var.name), var.tags)}"
